...
function timedWithWasmCalls(fn) {
  ...
  try {
    ...
    return {
      ...result,
      wasmCalls: calls.length,
      wasmCallHistogram: histogram(calls),
    };
  } finally {
...
function measurement(result, iterations, operationCount, dataBytes) {
  ...
  return {
    ...
    operationCount,
    dataBytes,
    operationsPerByte: round(operationCount / dataBytes, 6),
    gopsPerSecond: round(operationCount / seconds / 1e9, 6),
    gbPerSecond: round(dataBytes / seconds / 1e9, 6),
    wasmCalls: result.wasmCalls,
    wasmCallsPerIteration: round(result.wasmCalls / iterations, 6),
    wasmCallHistogram: result.wasmCallHistogram,
    checksum: round(result.checksum, 6),
  };
}

function firstLastGrowth(rows, key) {
  if (rows.length < 2) return 1;
  return rows[rows.length - 1][key] / rows[0][key];
}
...
function runWasmTorusAction(size, rhsCols, iterations) {
  const values = makeData(size, 1, 0x74a7c159, { min: 0.5, max: 2 });
  const diagonal = Matrix.diagonal(values);
  const right = Matrix.from(
    size,
    rhsCols,
...
function runDataComputeSweep(config) {
  const elementwise = config.sweepElementSizes.map((size) => {
    const iterations = config.sweepElementIterations;
    const elements = size * size;
    const operationCount = elements * 9 * iterations;
    const dataBytes = elements * 4 * 8 * iterations;
    const result = timedWithWasmCalls(() =>
      runWasmElementwise(size, size, iterations)
    );
    return {
      kind: "elementwise-dag",
      shape: [size, size],
      ...measurement(result, iterations, operationCount, dataBytes),
    };
  });

  const matmul = config.sweepMatmulSizes.map((size) => {
    const iterations = config.sweepMatmulIterations;
    const operationCount = 2 * size * size * size * iterations;
    const dataBytes = size * size * 4 * 3 * iterations;
    const result = timedWithWasmCalls(() => runWasmMatmul(size, iterations));
    return {
      kind: "dense-gemm",
      shape: [size, size],
      ...measurement(result, iterations, operationCount, dataBytes),
  ...
  const torusAction = config.sweepTorusSizes.map((size) => {
    const iterations = config.sweepTorusIterations;
    const rhsCols = config.sweepTorusRhsCols;
    const elements = size * rhsCols;
    const operationCount = elements * 2 * iterations;
    const dataBytes = (elements * 4 + size * 2) * 4 * iterations;
    const result = timedWithWasmCalls(() =>
      runWasmTorusAction(size, rhsCols, iterations)
    );
    return {
      kind: "diagonal-torus-action",
      shape: [size, rhsCols],
      ...measurement(result, iterations, operationCount, dataBytes),
    };
  });
  ...
  return {
    assumptions: {
      elementwiseOpsPerElement: 9,
      matmulOps: "2*n^3 fused multiply-add scalar operations",
      torusOpsPerElement: 2,
      dataBytes: "estimated input/output traffic, not hardware counter traffic",
    },
    elementwise,
    matmul,
    torusAction,
    analysis: {
      elementwiseIntensityGrowth: round(
        firstLastGrowth(elementwise, "operationsPerByte"),
        6,
      ),
      matmulIntensityGrowth: round(
        firstLastGrowth(matmul, "operationsPerByte"),
        6,
      ),
      torusIntensityGrowth: round(
        firstLastGrowth(torusAction, "operationsPerByte"),
        6,
      ),
      highestObservedGopsPerSecond: round(
        Math.max(
          ...elementwise.map((row) => row.gopsPerSecond),
          ...matmul.map((row) => row.gopsPerSecond),
          ...torusAction.map((row) => row.gopsPerSecond),
        ),
        6,
      ),
      highestObservedGbPerSecond: round(
        Math.max(
          ...elementwise.map((row) => row.gbPerSecond),
          ...matmul.map((row) => row.gbPerSecond),
          ...torusAction.map((row) => row.gbPerSecond),
        ),
        6,
      ),
    },
...
export function runBenchmarkSuite(config = defaultBenchmarkConfig()) {
  const {
    elementRows,
    elementCols,
    elementIterations,
  ...
  const summary = {
    ...
    dataComputeSweep: runDataComputeSweep(config),
  };
...
function installSilentConsole() {
  const captured = [];
  const methods = ["debug", "error", "info", "log", "table", "trace", "warn"];
...
if (workerUrl.searchParams.get("worker") === "1") {
  const capturedConsole = installSilentConsole();
  const workerScope = self as any;

  workerScope.postMessage({ ready: true });
  workerScope.onmessage = (event) => {
    try {
      const summary = runBenchmarkSuite(event.data);
      workerScope.postMessage({
        ok: true,
        summary,
        capturedConsoleCount: capturedConsole.length,
        capturedConsolePreview: capturedConsole.slice(0, 5),
      });
    } catch (error) {
      workerScope.postMessage({
        ok: false,
