...
import {
  ...
  LOCALES,
  uiContent,
} from "./src/i18n/siteContent.ts";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

const githubUrl = "https://github.com/ihasq/wasmatrix";
const npmUrl = "https://www.npmjs.com/package/wasmatrix";
const siteUrl = (process.env.DOCS_SITE_URL || process.env.SITE_URL ||
  "https://wasmatrix.pages.dev")
  .replace(/\/+$/, "");
      ...
      return {
        ...
        getClientModules() {
          return [
            path.resolve(
              __dirname,
              "src/components/wasmatrixSandboxElement.ts",
            ),
          ];
        },
      };
