...
keywords: [WASMatrix API, Matrix class, WebAssembly SIMD, AssemblyScript,
...
The default export is the `Matrix` class. Named exports include runtime helpers
...
This page is generated from the TSDoc comments in `index.d.ts`, which is copied
to `dist/index.d.ts` during the package build.
...
**Remarks:** Most users should use the default runtime initialized by the
package entrypoint. Separate runtimes are useful only when you intentionally
want isolated WASM memories and caches.
...
**Remarks:** Existing matrices keep their data, but cache sizing changes can
evict cached buffers immediately.
...
**Remarks:** Most operations return another `Matrix`. Data stays in WASM memory
until you call a readback method such as `Matrix.toArray`,
`Matrix.toFloat32Array`, `Matrix.row`, or `Matrix.column`.
...
**Remarks:** Diagonal structure is tracked for determinant, solve, inverse, and
...
**Remarks:** Identity structure is tracked so operations such as `I.matmul(A)`
can use structural shortcuts.
...
**Remarks:** The chain order is chosen by dynamic programming over matrix
dimensions. Intermediate products are materialized and disposed where possible.
...
**Remarks:** Array overload for callers that already hold the chain in a
collection.
...
**Remarks:** This increments the matrix version and invalidates cached
factorizations, reductions, materialized views, and packed operands.
...
**Remarks:** Row vectors have shape `1 x cols`; column vectors have shape
`rows x 1`. Elementwise chains are represented lazily and fused when
materialized.
...
**Remarks:** A numeric argument scales elementwise. A broadcast-compatible
matrix uses elementwise multiplication. Other matrix arguments use matrix
multiplication.
...
**Remarks:** Division follows JavaScript/WASM floating-point behavior for zero,
`NaN`, and infinities.
...
**Remarks:** This reads the matrix into JavaScript and writes a new WASM-backed
matrix. Prefer built-in elementwise methods when possible.
...
**Remarks:** The transposed buffer is materialized only when an operation
requires a concrete layout. `A.transpose().transpose()` collapses back to `A`.
...
**Remarks:** Transpose-aware kernels, diagonal shortcuts, identity/zero
propagation, inverse-view solves, Gram/SPD tags, and packed operand caches may
be used according to shape and structure.
...
**Remarks:** Both operands are flattened row-major and must have the same
length.
...
**Remarks:** For some lazy products, the trace can be computed without
materializing the full dense product.
...
**Remarks:** Uses structural shortcuts, Cholesky for SPD-tagged matrices when
available, and LU factorization otherwise. Results are cached by matrix version.
...
**Remarks:** Returns `NaN` when the available path determines that the
...
**Remarks:** The dense inverse is avoided until required. Expressions such as
...
**Remarks:** Square systems use diagonal, Cholesky, or LU paths depending on
structure. Factorizations are cached by matrix version and reused across
compatible `determinant`, `solve`, and `inverse` operations.
...
**Remarks:** Calling `dispose()` more than once is safe. Do not use the matrix
